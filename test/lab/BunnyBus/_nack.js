...
describe('BunnyBus', () => {
    ...
    describe('public methods', () => {
        describe('_nack', () => {
            const baseChannelName = 'bunnybus-nack';
            const baseQueueName = 'test-nack-queue';
            ...
            it('should nack a message off the queue', async () => {
                await Assertions.autoRecoverChannel(
                    async () => {
                        await instance.publish({ message });
                        const payload = await instance.get({ queue: baseQueueName });

                        await instance._nack({ payload, channelName: BunnyBus.QUEUE_CHANNEL_NAME(baseQueueName) });
