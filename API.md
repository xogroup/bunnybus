 ...
 * `dispatchType` - enumerated value to select dispatch mechanism used.  `serial` will flow messages to the your message handler(s) in single file.  `concurrent` will flow messages simultaneously to your message handler(s).  Defaults to `serial`.  *[string]* **Optional**
