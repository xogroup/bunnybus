            ...
            await Helpers.retryAsync(
                async () => {
                    ...
                    channelContext.channel
                        .on('close', () => channelContext.emit(Events.AMQP_CHANNEL_CLOSE_EVENT))
                        .on('error', (err) => channelContext.emit(Events.AMQP_CHANNEL_ERROR_EVENT, err))
                        .on('return', (payload) => channelContext.emit(Events.AMQP_CHANNEL_RETURN_EVENT, payload))
                        .on('drain', () => channelContext.emit(Events.AMQP_CHANNEL_DRAIN_EVENT));
                },
