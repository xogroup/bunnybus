...
class ConnectionManager extends EventEmitter {
                ...
                async () => {
                    ...
                    connectionContext.connection
                        .on('close', () => {
                            ...
                            connectionContext.emit(Events.AMQP_CONNECTION_CLOSE_EVENT, connectionContext);
                        })
                        .on('error', (err) => {

                            connectionContext.emit(Events.AMQP_CONNECTION_ERROR_EVENT, err, connectionContext);
                        })
                        .on('blocked', (reason) => {

                            connectionContext.blocked = true;
                            connectionContext.emit(Events.AMQP_CONNECTION_BLOCKED_EVENT, connectionContext, reason);
                        })
                        .on('unblocked', () => {

                            connectionContext.blocked = false;
                            connectionContext.emit(Events.AMQP_CONNECTION_UNBLOCKED_EVENT, connectionContext);
                        });
                },
    ...
    async remove(name) {
        ...
        if (this._connections.has(name)) {
            ...
            connectionContext.emit(Events.CONNECTION_MANAGER_REMOVED, connectionContext);
            this.emit(Events.CONNECTION_MANAGER_REMOVED, connectionContext);
        }
    }
