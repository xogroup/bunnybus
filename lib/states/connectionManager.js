...
class Connection extends EventEmitter {
    ...
    get healthy() {
        ...
        const overTimeLimit =
            lockTimeDiff > this.connectionOptions.timeout * this.connectionOptions.connectionRetryCount;

        return overTimeLimit ? (this.connection ? true : false) : true;
    }
}
        ...
        if (connectionContext.lock) {
            ...
            try {
                await Helpers.retryAsync(
                    ...
                    (err) => {
                        return err.code && err.code === 'ENOTFOUND' ? false : true;
                    }
                );
            } catch (err) {
