                ...
                async () => {
                    ...
                    connectionContext.connection
                        .on('close', () => {
                            ...
                            connectionContext.emit(Events.AMQP_CONNECTION_CLOSE_EVENT);
                        })
                        .on('error', (err) => {
                            
                            connectionContext.emit(Events.AMQP_CONNECTION_ERROR_EVENT, err);
                        })
                        .on('blocked', (reason) => {
                            
                            connectionContext.blocked = true;
                            connectionContext.emit(Events.AMQP_CONNECTION_BLOCKED_EVENT, reason);
                        })
                        .on('unblocked', () => {
                            
                            connectionContext.blocked = false;
                            connectionContext.emit(Events.AMQP_CONNECTION_UNBLOCKED_EVENT);
                        });
                },
