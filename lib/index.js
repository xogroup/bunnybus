...
class BunnyBus extends EventEmitter{
    ...
    static get DEFAULT_MANAGEMENT_CHANNEL_NAME() {
    ...
    async createExchange(name, type, options) {
        ...
        const channelContext = await this._autoBuildChannelContext(BunnyBus.DEFAULT_MANAGEMENT_CHANNEL_NAME);

        //type : (direct, fanout, header, topic)
        await channelContext.channel.assertExchange(name, type, Object.assign({}, BunnyBus.DEFAULT_EXCHANGE_CONFIGURATION, options));
    }
    ...
    async deleteExchange(name, options) {
        ...
        await channelContext.channel.deleteExchange(name, options);
    }
    ...
    async checkExchange(name) {
        ...
        try {
            ...
            await this._autoBuildChannelContext(BunnyBus.DEFAULT_MANAGEMENT_CHANNEL_NAME);
        }
    ...
    async _autoBuildChannelContext(channelName) {
        ...
        const $C = $._connections;
        const $D = $._channels;
        let connectionContext = undefined;
        let channelContext = $D.get(channelName);

        if (channelContext && channelContext.channel && channelContext.connectionContext.connection) {
            return channelContext;
        }

        connectionContext = await $C.create(BunnyBus.DEFAULT_CONNECTION_NAME, $.config);
        channelContext = await $D.create(channelName, connectionContext, $.config);
        ...
        return Async.retry({
            ...
            return Async.waterfall([
                ...
                (channel, cb) => {
                    ...
                    $.channel = channel;
                    return $.channel.prefetch($.config.prefetch, null, cb);
                }
            ], retryCB);
        }, (err) => {
