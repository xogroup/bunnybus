...
class BunnyBus {
    ...
    ///
    /// options : not ampqp { globalExcahnge, routeKey}
    ///
    publish(message, options, callback) {
        ...
        Async.auto({
            ...
            //cache the exchange
            create_exchange: ['initialize', (results, cb) => $.createExchange($.config.globalExchange, 'topic', null, cb)],
            publish        : ['create_exchange', (results, cb) => {

                //TODO
                //* create transactionId tracer
                //* track plugin name
