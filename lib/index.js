...
class BunnyBus extends EventEmitter {
    ...
    async getAll(queue, handler, options) {
        ...
        const exec = async () => {
            ...
            const ack = this._ack.bind(null, payload);
            meta
    ...
    async _consume({ queue, payload, options, handlers }) {
        ...
        const ack = this._ack.bind(null, payload);
        const reject = this._reject.bind(null, payload, errorQueue);
        const requeue = meta
    ...
    async _autoConnectChannel() {
        ...
        let retryCount = 0;
        const condition = () =>
            !(this.hasConnection && this.hasChannel) &&
            retryCount < this.config.autoRecoveryRetryCount;
