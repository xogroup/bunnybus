...
class BunnyBus extends EventEmitter{
    ...
    async _recoverConnection() {
        ...
        $.emit(BunnyBus.RECOVERING_CONNECTION_EVENT);

        if (!$._state.connectionRecoveryLock) {

            $._state.connectionRecoveryLock = true;

            try {
                const promises = [];

                for (const { queue, handlers, options } of $.subscriptions.list()) {
                    if (!$.subscriptions.isBlocked(queue)) {
                        promises.push($.subscribe(queue, handlers, options));
            ...
            $._state.connectionRecoveryLock = false;
        }

        $.emit(BunnyBus.RECOVERED_CONNECTION_EVENT);
    }

    async _recoverChannel(channelName) {

        $.emit(BunnyBus.RECOVERING_CHANNEL_EVENT);

        if (!$._state.channelRecoveryLock) {

            $._state.channelRecoveryLock = true;
            ...
            $._state.channelRecoveryLock = false;
        }

        $.emit(BunnyBus.RECOVERED_CHANNEL_EVENT);
    }
}
...
internals.handlers[ConnectionManager.AMQP_CONNECTION_ERROR_EVENT] = (err, context) => {
    ...
    // need to remove connection from manager
};

internals.handlers[ConnectionManager.AMQP_CONNECTION_CLOSE_EVENT] = (context) => {
...
internals.handlers[ChannelManager.AMQP_CHANNEL_ERROR_EVENT] = (err, context) => {
    ...
    // need to remove channel from manager
};

internals.handlers[ChannelManager.AMQP_CHANNEL_CLOSE_EVENT] = (context) => {

    $.logger.info('channel closed', context);

    $._recoverChannel(context.name);
};
