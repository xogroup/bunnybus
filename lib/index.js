...
class BunnyBus extends EventEmitter {
    constructor(config) {
        ...
        this._httpClients = new HttpClientManager();
        this._dispatchers = {
        ...
        this._subscriptions.on(
            ...
            this._on_subscriptionManager_BLOCKED_EVENT.bind(this)
        );
        this._subscriptions.on(
            SubscriptionManager.UNBLOCKED_EVENT,
            this._on_subscriptionManager_UNBLOCKED_EVENT.bind(this)
        );
        ...
        setInterval(() => {
            this.logger.info(
                `Connection Count ${this._connections.list().length} && Channel Count ${this._channels.list().length}`
            );
        }, 30000);
    ...
    static get DEFAULT_HTTP_CLIENT_NAME() {
        return 'default';
    }
    ...
    async createQueue(name, options) {
        if (!this.config.disableQueueCreate) {
            ...
            return await channelContext.channel.assertQueue(
                name,
                Object.assign({}, BunnyBus.DEFAULT_QUEUE_CONFIGURATION, options)
            );
        }
    }
    ...
    async checkQueue(name) {
        ...
        if (httpClient) {
            try {
                result = await httpClient.getQueue(name);
            } catch (err) {
                if (err.status !== 404) {
                    throw err;
                }
    ...
    async subscribe(queue, handlers, options) {
        ...
        const validatePublisher =
            options && options.hasOwnProperty('validatePublisher')
                ? options.validatePublisher
                : this.config.validatePublisher;
        const validateVersion =
            options && options.hasOwnProperty('validateVersion')
                ? options.validateVersion
                : this.config.validateVersion;
        const disableQueueBind =
            options && options.hasOwnProperty('disableQueueBind')
                ? options.disableQueueBind
                : this.config.disableQueueBind;
        const rejectUnroutedMessages =
        ...
        if (!disableQueueBind) {
            await Promise.all(
                Object.keys(handlers).map((pattern) => channelContext.channel.bindQueue(queue, globalExchange, pattern))
            );
        }
    ...
    async _autoBuildChannelContext(channelName, queue = null) {
        ...
        addListener(
            ...
            this._on_channelManager_AMQP_CHANNEL_CLOSE_EVENT.bind(this)
        );
        addListener(
            channelContext,
            ChannelManager.AMQP_CHANNEL_ERROR_EVENT,
            this._on_channelManager_AMQP_CHANNEL_ERROR_EVENT.bind(this)
        );
