...
class BunnyBus extends EventEmitter{
    ...
    static SEND_CHANNEL_NAME(queueName) {

        return `send-${queueName}-channel`;
    }
    ...
    async send(message, queue, options) {
        ...
        const [convertedMessage, transactionId, channelContext] = await Promise.all([
            ...
            $._autoBuildChannelContext(BunnyBus.MANAGEMENT_CHANNEL_NAME()),
            $.createQueue(queue)
        ]);
        // const convertedMessage = await Helpers.convertToBufferAsync(message);
        ...
        await channelContext.waitForConfirms();
