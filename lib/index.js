...
class BunnyBus extends EventEmitter{
    ...
    static SEND_CHANNEL_NAME(queue) {

        return `send-${queue}-channel`;
    }

    static RECEIVE_CHANNEL_NAME(queue) {

        return `receive-${queue}-channel`;
    }
    ...
    async send(message, queue, options) {
        ...
        const [convertedMessage, transactionId, channelContext] = await Promise.all([
            ...
            $._autoBuildChannelContext(BunnyBus.SEND_CHANNEL_NAME(queue)),
            $.createQueue(queue)
        ]);
    ...
    async get(queue, options) {
        ...
        const channelContext = await $._autoBuildChannelContext(BunnyBus.RECEIVE_CHANNEL_NAME(queue));
            ...
            if (payload) {
                ...
                if (meta) {
                    await handler(
                        ...
                        $._ack.bind(null, payload, BunnyBus.RECEIVE_CHANNEL_NAME(queue))
                    );
                }
                else {
                    await handler(
