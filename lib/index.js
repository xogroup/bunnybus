...
const { ChannelManager, ConnectionManager, Dispatcher, SubscriptionManager } = require('./states');
                    ...
                    if (matchedHandlers.length > 0) {
                        ...
                        // check for `bunnyBus`:<version> semver
                            ...
                            matchedHandlers.forEach(async (matchedHandler) => {
                                ...
                                // amqplib consume does not handle asynchronous calls so handlers will end
                                // out of order if operation length are not equal for every message.
                                $._dispatcher.push(
                                    queue,
                                    async () => {
