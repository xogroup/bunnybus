...
class BunnyBus {
    ...
    createChannel(callback) {
        ...
        Async.auto({
            ...
            set_prefetch: ['create_channel', (results, cb) => {

                if (!$.hasChannel) {
                    results.create_channel.prefetch($.config.prefetch, null, (err) => {
            ...
            store_channel: ['create_channel', (results, cb) => {

                if (!$.hasChannel) {
                    $.channel = results.create_channel;
                }

                cb();
            }]
        }, (err) => callback(err)); //integration 106
    }
    ...
    ///
    /// options : not ampqp { globalExcahnge, routeKey, transactionId, callingModule}
    ///
