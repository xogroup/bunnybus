...
class BunnyBus extends EventEmitter {
    ...
    async checkExchange(name) {
        ...
        const channelCloseEventPromise = new Promise((resolve) => {
        ...
        try {
            await Helpers.timeoutAsync(async () => await channelCloseEventPromise, 500);
        }
        catch (err) {
    ...
    async send(message, queue, options) {
        ...
        channelContext.channel.sendToQueue(queue, convertedMessage.buffer, sendOptions);
    ...
    async publish(message, options) {
        ...
        const convertedMessage = Helpers.convertToBuffer(message);
        const transactionId = (options && options.transactionId) ? options.transactionId : Helpers.createTransactionId();
        const channelContext = await $._autoBuildChannelContext(BunnyBus.PUBLISH_CHANNEL_NAME());
        await $.createExchange(globalExchange, 'topic', null);
        ...
        channelContext.channel.publish(globalExchange, routeKey, convertedMessage.buffer, publishOptions);
        await channelContext.channel.waitForConfirms();
    ...
    async subscribe(queue, handlers, options) {
        ...
        const normalizeMessages = (options && options.hasOwnProperty('normalizeMessages')) ? options.normalizeMessages : $.config.normalizeMessages;
        const meta = (options && options.meta);
        const channelName = BunnyBus.QUEUE_CHANNEL_NAME(queue);

        const channelContext = await $._autoBuildChannelContext(channelName, queue);
        //@TODO check if this needs to be broken out
        await Promise.all([
        ...
        const result = await channelContext.channel.consume(
            ...
            async (payload) => {
                ...
                if (payload) {
                    const parsedPayload = Helpers.parsePayload(payload, normalizeMessages);
    ...
    async _requeue(payload, channelName, queue, options) {
        ...
        channelContext.channel.sendToQueue(queue, payload.content, sendOptions);
        await channelContext.channel.waitForConfirms();
        channelContext.channel.ack(payload);
